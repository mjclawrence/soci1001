...
How could we see this plot separately for metro, micro, and rural areas (categories of the `type`) variable?

```{r}
county_mobility %>%
  ggplot(aes(x = social_capital_90, y = absolute_upward_mobility)) +
  geom_point() + facet_grid(type~.)
```

We might be interested in seeing the correlations between mobility and the variables considered the "big five" predictors of mobility. How can we see summarize all those correlations together? The `GGally` package has some neat tools to help visualize correlations. Install and load the package.
